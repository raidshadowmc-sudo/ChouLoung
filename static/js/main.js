...
// Enhanced event delegation with browser compatibility
    document.addEventListener('click', function(e) {
        var target = e.target;
        // Check if target matches using compatible method
...
function updateRolePreview() {
    ...
    if (useGradient) {
        ...
        if (isAnimated) {
            roleDisplay.style.animation = 'gradientShift 3s ease infinite';
        } else {
...
// Apply custom knife cursor based on theme
function applyCustomCursor() {
    const body = document.body;
    
    // Remove existing cursor classes
    body.classList.remove('custom-cursor', 'theme-default', 'theme-cyber_matrix', 'theme-royal_purple', 'theme-ocean_depths', 'theme-volcano_fire', 'theme-midnight_blue');
    
    // Add custom cursor base class
    body.classList.add('custom-cursor');
    
    // Get current theme from session or default
    const currentTheme = sessionStorage.getItem('current_theme') || 'default';
    ...
    console.log('🎯 Custom knife cursor applied with theme:', currentTheme);
}

// Apply cursor when page loads
document.addEventListener('DOMContentLoaded', function() {
    applyCustomCursor();
});

// Apply cursor when theme changes
