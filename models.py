...
class Player(db.Model):
    ...
    def get_purchased_items(self):
        """Get all purchased items from shop"""
        from models import ShopPurchase, ShopItem
        
        purchases = ShopPurchase.query.filter_by(
            player_id=self.id,
            is_active=True
        ).all()
    ...
    def has_purchased_item(self, item_id):
        """Check if player has purchased specific item"""
        from models import ShopPurchase
        
        return ShopPurchase.query.filter_by(
            player_id=self.id,
            item_id=item_id,
            is_active=True
        ).first() is not None
...
class ShopItem(db.Model):
    ...
    def apply_item_effect(self, player):
        ...
        try:
            ...
            elif self.category == 'multiplier_booster':
                # Create multiplier booster
                from models import PlayerBooster
                
                booster = PlayerBooster(
                    player_id=player.id,
                    booster_type=data.get('booster_type', 'coins'),
    ...
    def create_default_items(cls):
        ...
        default_items = [
            ...
            # Multiplier Boosters
            {
                'name': 'coins_multiplier_2x',
                'display_name': 'Множитель койнов x2',
                'description': 'Удвойте получаемые койны на 24 часа',
                'category': 'multiplier_booster',
                'price_coins': 10000,
                'price_reputation': 200,
                'unlock_level': 25,
                'rarity': 'epic',
                'item_data': '{"booster_type": "coins", "multiplier": 2.0, "duration_hours": 24}'
            },
            {
                'name': 'coins_multiplier_3x',
                'display_name': 'Множитель койнов x3',
                'description': 'Утройте получаемые койны на 12 часов',
                'category': 'multiplier_booster',
                'price_coins': 25000,
                'price_reputation': 500,
                'unlock_level': 40,
                'rarity': 'legendary',
                'item_data': '{"booster_type": "coins", "multiplier": 3.0, "duration_hours": 12}'
            },
            {
                'name': 'reputation_multiplier_2x',
                'display_name': 'Множитель репутации x2',
                'description': 'Удвойте получаемую репутацию на 24 часа',
                'category': 'multiplier_booster',
                'price_coins': 15000,
                'price_reputation': 0,
                'unlock_level': 30,
                'rarity': 'epic',
                'item_data': '{"booster_type": "reputation", "multiplier": 2.0, "duration_hours": 24}'
            },
            {
                'name': 'reputation_multiplier_3x',
                'display_name': 'Множитель репутации x3',
                'description': 'Утройте получаемую репутацию на 12 часов',
                'category': 'multiplier_booster',
                'price_coins': 35000,
                'price_reputation': 0,
                'unlock_level': 50,
                'rarity': 'legendary',
                'item_data': '{"booster_type": "reputation", "multiplier": 3.0, "duration_hours": 12}'
            },
            {
                'name': 'combo_multiplier_2x',
                'display_name': 'Комбо множитель x2',
                'description': 'Удвойте койны и репутацию на 18 часов',
                'category': 'multiplier_booster',
                'price_coins': 50000,
                'price_reputation': 1000,
                'unlock_level': 60,
                'rarity': 'mythic',
                'item_data': '{"booster_type": "both", "multiplier": 2.0, "duration_hours": 18}'
            },
...
class PlayerBooster(db.Model):
    """Player boosters with multipliers for coins and reputation"""
    
    id = db.Column(db.Integer, primary_key=True)
    player_id = db.Column(db.Integer, db.ForeignKey('player.id'), nullable=False)
    booster_type = db.Column(db.String(50), nullable=False)  # coins, reputation, both
    multiplier = db.Column(db.Float, nullable=False, default=2.0)  # 2.0 = 2x, 3.0 = 3x
    duration_hours = db.Column(db.Integer, nullable=False, default=24)  # Duration in hours
    activated_at = db.Column(db.DateTime, nullable=True)  # When booster was activated
    expires_at = db.Column(db.DateTime, nullable=True)  # When booster expires
    is_active = db.Column(db.Boolean, default=False)  # Is currently active
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    
    # Relationships
    player = db.relationship('Player', backref='boosters')
    
    def __repr__(self):
        return f'<PlayerBooster {self.booster_type} x{self.multiplier} for {self.player.nickname}>'
    ...
    def activate(self):
        ...
        if self.is_active:
            return False, "Бустер уже активен"
        
        # Deactivate other boosters of the same type
        PlayerBooster.query.filter_by(
            player_id=self.player_id,
        ...
        db.session.commit()
        return True, f"Бустер {self.booster_type} x{self.multiplier} активирован на {self.duration_hours} часов"
    ...
    def deactivate(self):
        ...
        db.session.commit()
        return True, "Бустер деактивирован"
    ...
    @classmethod
    def get_booster_multiplier(cls, player_id, booster_type):
        """Get current multiplier for a specific booster type"""
        active_booster = cls.query.filter_by(
            player_id=player_id,
            booster_type=booster_type,
            is_active=True
        ).filter(cls.expires_at > datetime.utcnow()).first()
        
        return active_booster.multiplier if active_booster else 1.0