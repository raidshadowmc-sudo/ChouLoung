...
def admin_modify_stats():
    ...
    try:
        ...
        if not all([player_nickname, stat_type, operation, value >= 0]):
        ...
        if stat_type == 'experience':
            player.level = player.calculate_level()

        db.session.commit()

        # Log the change
        app.logger.info(f"Admin modified {player_nickname} {stat_type}: {current_value} -> {new_value} (operation: {operation})")
...
def bulk_update_ascend():
    ...
    try:
        ...
        for update_data in updates:
            ...
            if 'pvp_tier' in update_data:
                ascend_data.pvp_tier = update_data['pvp_tier']
            if 'clutching_tier' in update_data:
                ascend_data.clutching_tier = update_data['clutching_tier']
            if 'block_placement_tier' in update_data:
                ascend_data.block_placement_tier = update_data['block_placement_tier']
            if 'gamesense_tier' in update_data:
                ascend_data.gamesense_tier = update_data['gamesense_tier']
            if 'overall_tier' in update_data:
                ascend_data.overall_tier = update_data['overall_tier']
            
            # Update scores if provided
            if 'pvp_score' in update_data:
                ascend_data.pvp_score = max(0, min(100, update_data['pvp_score']))
            if 'clutching_score' in update_data:
                ascend_data.clutching_score = max(0, min(100, update_data['clutching_score']))
            if 'block_placement_score' in update_data:
                ascend_data.block_placement_score = max(0, min(100, update_data['block_placement_score']))
            if 'gamesense_score' in update_data:
                ascend_data.gamesense_score = max(0, min(100, update_data['gamesense_score']))
            
            # Update other fields if provided
            if 'comment' in update_data:
                ascend_data.comment = update_data['comment']
            if 'evaluator_name' in update_data:
                ascend_data.evaluator_name = update_data['evaluator_name']
            
            updated_count += 1
        
        db.session.commit()
        
        app.logger.info(f"Bulk ASCEND update completed for {updated_count} players by admin {session.get('player_nickname', 'Unknown')}")
        
        return jsonify({
            'success': True,
            'message': f'ASCEND данные обновлены для {updated_count} игроков',
...
# Item Activation API Routes
@app.route('/api/items/activate-theme', methods=['POST'])
def activate_theme():
    """Activate a purchased theme"""
    if not session.get('player_nickname'):
        return jsonify({
            'success': False,
    ...
    try:
        ...
        if not theme_id:
            return jsonify({
                'success': False,
                'error': 'ID темы не указан'
            }), 400
        
        # Get current player
        player = Player.query.filter_by(nickname=session['player_nickname']).first()
        if not player:
            return jsonify({
                'success': False,
        ...
        # Check if player has purchased this theme
        if not player.has_purchased_item(theme_id):
            return jsonify({
                'success': False,
                'error': 'Тема не куплена'
            }), 403
        
        # Get theme
        from models import SiteTheme
        theme = SiteTheme.query.get(theme_id)
        if not theme:
            return jsonify({
                'success': False,
        ...
        app.logger.info(f"Theme '{theme.display_name}' activated for player {player.nickname}")
        
        return jsonify({
            'success': True,
            'message': f'Тема "{theme.display_name}" активирована',
            'theme': {
                'id': theme.id,
                'name': theme.name,
                'display_name': theme.display_name,
        ...
        db.session.rollback()
        app.logger.error(f"Error activating theme: {e}")
        return jsonify({
            'success': False,
            'error': f'Ошибка при активации темы: {str(e)}'
        }), 500

@app.route('/api/items/activate-cursor', methods=['POST'])
def activate_cursor():
    """Activate a purchased cursor"""
    if not session.get('player_nickname'):
        return jsonify({
            'success': False,
            'error': 'Необходимо войти в систему'
    ...
    try:
        ...
        # Check if player has purchased this cursor
        if not player.has_purchased_item(cursor_id):
            return jsonify({
                'success': False,
                'error': 'Курсор не куплен'
            }), 403
        
        # Get cursor
        from models import CursorTheme
        cursor = CursorTheme.query.get(cursor_id)
        if not cursor:
            return jsonify({
                'success': False,
                'error': 'Курсор не найден'
            }), 404
        
        # Activate cursor
        player.selected_cursor_id = cursor_id
        db.session.commit()
        
        app.logger.info(f"Cursor '{cursor.display_name}' activated for player {player.nickname}")
        
        return jsonify({
            'success': True,
            'message': f'Курсор "{cursor.display_name}" активирован',
            'cursor': {
                'id': cursor.id,
                'name': cursor.name,
                'display_name': cursor.display_name,
                'cursor_url': cursor.cursor_url,
                'preview_url': cursor.preview_url
            }
        })
        
    except Exception as e:
        db.session.rollback()
        app.logger.error(f"Error activating cursor: {e}")
        return jsonify({
            'success': False,
            'error': f'Ошибка при активации курсора: {str(e)}'
        }), 500

@app.route('/api/items/activate-title', methods=['POST'])
def activate_title():
    """Activate a purchased title"""
    if not session.get('player_nickname'):
        return jsonify({
            'success': False,
            'error': 'Необходимо войти в систему'
    ...
    try:
        ...
        # Check if player has purchased this title
        if not player.has_purchased_item(title_id):
            return jsonify({
                'success': False,
                'error': 'Титул не куплен'
            }), 403
        ...
        app.logger.info(f"Title '{title.display_name}' activated for player {player.nickname}")
        
        return jsonify({
            'success': True,
            'message': f'Титул "{title.display_name}" активирован',
            'title': {
                'id': title.id,
                'name': title.name,
                'display_name': title.display_name,
                'color': title.color,
                'glow_color': title.glow_color
            }
        })
        ...
        return jsonify({
            'success': False,
            'error': f'Ошибка при активации титула: {str(e)}'
        }), 500

@app.route('/api/items/activate-booster', methods=['POST'])
def activate_booster():
    """Activate a purchased multiplier booster"""
    if not session.get('player_nickname'):
        return jsonify({
            'success': False,
            'error': 'Необходимо войти в систему'
    ...
    try:
        ...
        # Get booster
        from models import PlayerBooster
        booster = PlayerBooster.query.get(booster_id)
        if not booster:
            return jsonify({
                'success': False,
                'error': 'Бустер не найден'
            }), 404
        
        # Check if booster belongs to player
        if booster.player_id != player.id:
            return jsonify({
                'success': False,
                'error': 'Бустер не принадлежит вам'
            }), 403
        
        # Activate booster
        success, message = booster.activate()
        
        if success:
            app.logger.info(f"Booster {booster.booster_type} x{booster.multiplier} activated for player {player.nickname}")
            ...
            return jsonify({
                ...
                'booster': {
                    ...
                    'expires_at': booster.expires_at.isoformat() if booster.expires_at else None
                }
            })
        else:
            return jsonify({
        ...
        db.session.rollback()
        app.logger.error(f"Error activating booster: {e}")
        return jsonify({
            'success': False,
            'error': f'Ошибка при активации бустера: {str(e)}'
        }), 500

@app.route('/api/items/deactivate-booster', methods=['POST'])
def deactivate_booster():
    """Deactivate an active booster"""
    if not session.get('player_nickname'):
        return jsonify({
            'success': False,
            'error': 'Необходимо войти в систему'
    ...
    try:
        ...
        # Deactivate booster
        success, message = booster.deactivate()
        
        if success:
            app.logger.info(f"Booster {booster.booster_type} x{booster.multiplier} deactivated for player {player.nickname}")
        ...
        db.session.rollback()
        app.logger.error(f"Error deactivating booster: {e}")
        return jsonify({
            'success': False,
            'error': f'Ошибка при деактивации бустера: {str(e)}'
        }), 500

@app.route('/api/items/get-active-boosters')
def get_active_boosters():
    """Get active boosters for current player"""
    if not session.get('player_nickname'):
        return jsonify({
            'success': False,
            'error': 'Необходимо войти в систему'
    ...
    try:
        ...
        # Get active boosters
        from models import PlayerBooster
        active_boosters = PlayerBooster.get_active_boosters(player.id)
        
        boosters_data = []
        for booster in active_boosters:
            boosters_data.append({
                'id': booster.id,
                'type': booster.booster_type,
                'multiplier': booster.multiplier,
                'time_remaining': booster.time_remaining,
                'expires_at': booster.expires_at.isoformat() if booster.expires_at else None
            })
        
        return jsonify({
            'success': True,
            'boosters': boosters_data
        })
        
    except Exception as e:
        app.logger.error(f"Error getting active boosters: {e}")
        return jsonify({
            'success': False,
            'error': f'Ошибка при получении бустеров: {str(e)}'
        }), 500